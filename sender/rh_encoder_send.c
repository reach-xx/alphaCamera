...
static void InitAudioParam(AudioParam *audio)
{
	...
	///0---8KHz
	///2---44.1KHz 闊抽閲囨牱鐜?(榛樿涓?4100)
	///1---32KHz
	///3---48KHz
	///鍏朵粬-----96KHz
	audio->SampleRate = 3;
	audio->Channel = 2;					//澹伴亾鏁?(榛樿涓?)
	audio->SampleBit = 16;				//閲囨牱浣?(榛樿涓?6)
	audio->BitRate = 12800;				//缂栫爜甯﹀ (榛樿涓?4000)
	audio->InputMode = LINE_INPUT;		//绾挎�ц緭鍏?	audio->RVolume = 31;					//宸﹀彸澹伴亾闊抽噺
	audio->LVolume = 31;
}
